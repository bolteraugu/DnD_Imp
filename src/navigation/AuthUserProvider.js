            ...
            value={{
                ...
                forgotPassword: async (email) => {
                    try {
                        ...
                        firebase.firestore().collection("user-logs").doc("user-creation").collection("logs").doc(email).get().then(
                            (snapshot) => {
                ...
                onGoogleButtonPress: async (rememberMe) => {
                            try {
                                ...
                                if (type === "success") {
                                    ...
                                    let docToConsider = firebase.firestore().collection("user-logs").doc("user-creation").collection("logs").doc(user.user.toJSON().email)
                                    let docExists = (await docToConsider.get()).exists

                                    const credential = firebase.auth.GoogleAuthProvider.credential(token)
                                    await firebase.auth().signInWithCredential(credential).then(() => {
                                        if (!docExists) {
                                            docToConsider.get().then( (snapshot) => {
                                                setStoredAccountType(snapshot.get("accountType"))}).then(() => {
                                                docToConsider.set({
                                                    text: `User ${user.user.toJSON().email} was created.`,
                                                    accountType: 'Google',
                                                    createdOn: new Date().toString()
