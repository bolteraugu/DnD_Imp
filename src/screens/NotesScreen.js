...
export default function NotesScreen({ route }) {
  ...
  useEffect(() => {
    const notesListener = charRef.collection("notes").onSnapshot( //Pretty much gets a snapshot of the notes from the currently looked at character
      (querySnapshot) => { //Query this snapshot
  ...
  if (loading) { //Show spinner if we are still getting the data.
    return <Spinner />;
  }
    ...
    <Provider>
      <View style={styles.wrapper}>
        <FlatList
          ...
          renderItem={({ item }) => ( //Render each item with the title and content
        ...
        <Portal>
          <Dialog visible={visible} onDismiss={hideDialog}>
            ...
            <Dialog.Actions>
              ...
              <Button
                onPress={() => { //If add button is pressed add the note to the notes collection in the firebase of the specified character
                  charRef
                    .collection("notes")
                    .add({
...
const styles = StyleSheet.create({
  fab: { //Absolute so it ignores it's parents positoning and instead is positioned bottom left corner with a margin of 20.
